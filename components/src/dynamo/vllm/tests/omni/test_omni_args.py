...
def test_resident_dlo_layers_reject_allgather():
    config = _make_omni_config(dlo_resident_layers=1, dlo_use_allgather=True)
    with pytest.raises(ValueError, match="--no-dlo-use-allgather"):
        config.validate()


def test_resident_dlo_layers_allow_sharded_reconstruction():
    config = _make_omni_config(dlo_resident_layers=1, dlo_use_allgather=False)
    config.validate()
