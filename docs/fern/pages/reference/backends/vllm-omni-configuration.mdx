...
<ParamField path="--task-type" type="string">
  Model-defined startup task partition. MiniMax-H3 accepts `fl2va` or `ref2va`;
  omit this flag to load its combined partition.
</ParamField>
...
<ParamField path="--diffusion-attention-config" type="json">
  vLLM-Omni per-role diffusion attention configuration.
</ParamField>
...
<ParamField path="--enable-distributed-layerwise-offload" type="flag">
  Enable distributed layerwise DiT offload.
</ParamField>
<ParamField path="--dlo-use-allgather" type="boolean" default="true">
  Reconstruct distributed layerwise-offload shards with AllGather.
</ParamField>
<ParamField path="--dlo-resident-layers" type="int" default="0">
  Number of leading main-DiT blocks kept resident when distributed layerwise
  offload is enabled.
</ParamField>
